...
class IndexHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        with open('web/Play.html', 'w') as myFile:
            txt_head='<!DOCTYPE html><html lang="en"><head><title>RTP Analysis</title><meta charset="utf-8"><link rel="stylesheet" href="css/libs/materialize.min.css"><script src="js/libs/jquery.min.js"></script><script src="js/libs/materialize.min.js"></script><script src="js/libs/hashtable.js"></script><script src="js/common.js"></script><script src="js/index.js"></script><link rel="stylesheet" href="css/index.css"></head>'
            txt_body1='<body><div class="container-fluid" id="mainPanel"><div id="scriptsPanel"><div id="scriptsList"><div id="listHeader"><h2 class="header" id="serverHeader">List Scripts</h2></div> <div id="scripts" class="collection">' \
                      '<a class="collection-item waves-effect waves-teal active" href="http://localhost:5000/audio.html">Audio Server</a>' \
                      '<a class="collection-item waves-effect waves-teal" href="http://localhost:5000/index.html#MOS_Score">MOS Score</a>' \
                       '<a class="collection-item waves-effect waves-teal" href="http://localhost:5000/index.html#RTP_Analyser">RTP Analyser</a></div>'
            txt_body2='<div id="scripts" class="collection">'
            myFile.write(txt_head)
            myFile.write(txt_body1)
            myFile.write(txt_body2)
            myFile.write('</div><div id="logoutPanel"><span id="usernameField"></span><input id="logoutButton" type="image" src="images/logout.png"></div></div>')
            ...
            for i in range(0, len(items), 1):
                if 'mp3' in items[i] or 'wav' in items[i]:
                    print(i, items[0])
                    myFile.write('<pre>' +' Call-UUID' + ' : ' + items[i] + '</pre>')
                    myFile.write('<p>')
            ...
            myFile.write('</body>')
            myFile.write('</html>')'''
        #self.render('web/a.html',title="My title",items=items)
    ...
    def post(self):
        ...
        try:
            ...
            if not config.redirect:
                ...
                IndexHandler.redirect(self,url='/audio.html',permanent="false")
...
def main():
    ...
    handlers = [(r"/scripts/list", GetScripts),
                ...
                (r"/", tornado.web.RedirectHandler, {"url": "/audio.html"})]
    if auth.is_enabled():
